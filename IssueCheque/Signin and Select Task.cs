...
namespace IssueCheque
{
    public partial class Signin_and_Select_Task : Form
    {
        ...
        {
            return
                System.Configuration.ConfigurationManager.ConnectionStrings["IssueCheque.Properties.Settings.Database1ConnectionString"].ConnectionString;
        }
        ...
        tryagainSignedIn:
            ...
            sqlRC = "select * from tbl_ChequeData";
        ...
        tryagainRC:
            try
            {
                ...
                adapterRC.Fill(dsRC, "tbl_ChequeData");
                adapterRC.Dispose();
                commandRC.Dispose();
                connectionRC.Close();
        ...
        private void pbSigninM_Click(object sender, EventArgs e)
        {
            ...
            if ((txtUserName.Text == "a") && (txtPassword.Text == "a"))
            {
                ...
                string connetionStringRCFT = null;
                SqlConnection connectionRCFT;
                SqlCommand commandRCFT;
                SqlDataAdapter adapterRCFT = new SqlDataAdapter();
                DataSet dsRCFT = new DataSet();
                string sqlRCFT = null;

                connetionStringRCFT = "Data Source=.\\SQLEXPRESS;AttachDbFilename=|DataDirectory|\\Database1.mdf;Integrated Security=True;User Instance=True";
                sqlRCFT = "select * from tbl_ChequeData where DOP = '" + Today + "'";
                connectionRCFT = new SqlConnection(connetionStringRCFT);

            tryagainRCFT:
                try
                {
                    connectionRCFT.Open();
                    commandRCFT = new SqlCommand(sqlRCFT, connectionRCFT);
                    adapterRCFT.SelectCommand = commandRCFT;
                    adapterRCFT.Fill(dsRCFT, "tbl_ChequeData");
                    adapterRCFT.Dispose();
                    commandRCFT.Dispose();
                    connectionRCFT.Close();
                }
                catch
                {
                    goto tryagainRCFT;
                ...
                if (dsRCFT.Tables[0].Rows.Count == 0)
                {
                    radioButton2.Enabled = false;
                }
                else
